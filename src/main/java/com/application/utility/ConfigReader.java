...
public class ConfigReader {
	...
	public static synchronized String getConfig(String config) {
		...
		if (readConfig("common.properties", "ENV").equalsIgnoreCase("qa2")) {
			return readConfig("qa2.properties", config);
		}
		...
		if (readConfig("common.properties", "ENV").equalsIgnoreCase("upmc-stage")) {
			return readConfig("upmc-stage.properties", config);
		}
		if (readConfig("common.properties", "ENV").equalsIgnoreCase("upmc-prod")) {
			return readConfig("upmc-prod.properties", config);
		}
